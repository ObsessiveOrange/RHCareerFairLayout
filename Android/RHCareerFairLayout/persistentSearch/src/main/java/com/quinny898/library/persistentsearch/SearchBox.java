...
public class SearchBox extends RelativeLayout {
	...
	private ImageView drawerLeftIcon;
	...
	private boolean animateDrawerLeftIcon = true;
	...
	private boolean hideAfterSearch = true;
	...
	public SearchBox(Context context, AttributeSet attrs, int defStyle) {
		...
		this.drawerLeftIcon = (ImageView) findViewById(R.id.drawer_left_icon);
		...
		results.setAdapter(new SearchAdapter(context, new ArrayList<SearchResult>()));
		animate = true;
	...
	public void revealFromMenuItem(int id, Activity activity) {
		...
		if (menuButton != null) {
			...
			if (layout.findViewWithTag("searchBox") == null) {
				...
				revealFrom(location[0] + menuButton.getWidth() * 2 / 3, location[1],
						activity, this);
			}
		}
	}
	...
	/***
	 * Set the image drawable of the drawer's left icon (do not set if you have not hidden the menu icon)
	 * @param icon Icon
	 */
	public void setDrawerLeftIcon(Drawable icon) {
		drawerLeftIcon.setImageDrawable(icon);
	}

	public void setDrawerLeftIcon(Integer icon) {
		setDrawerLeftIcon(getResources().getDrawable(icon));
	}
	...
	public int getNumberOfResults() {
		return results.getAdapter().getCount();
	}
    ...
    /***
     * Set to false to retain the icon from setDrawerLeftIcon() instead of animating to the arrow during searches.
     * @param show Should the SearchBox animate the drawer icon
     */
    public void setAnimateDrawerLeftIcon(boolean show){
        animateDrawerLeftIcon = show;
    }

	private void openSearch(Boolean openKeyboard) {
        if(animateDrawerLeftIcon){
            this.materialMenu.animateState(IconState.ARROW);
            this.drawerLeftIcon.setVisibility(View.GONE);
        }
		...
		results.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
                                    long arg3) {
                SearchResult result = ((SearchAdapter)results.getAdapter()).getItem(arg2);
                search(result);
	...
	private void setInitialResults(){
		...
		for (int x = 0; x < initialResults.size(); x++) {
			SearchResult searchable = initialResults.get(x);
	...
	private void closeSearch() {
        if(animateDrawerLeftIcon){
            ...
            this.drawerLeftIcon.setVisibility(View.VISIBLE);
        }
		this.search.setVisibility(View.GONE);
		this.results.setVisibility(View.GONE);
	...
	public boolean willHideAfterSearch() {
