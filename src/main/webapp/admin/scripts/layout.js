...
function populateCompanyList() {
    Object.keys(careerFairData.companies).forEach(function(companyId) {
        ...
        $("#companyListContainer").append("<tr class=level2' id='companyListRow_" + companyId + "'><td id='companyListName_" + companyId + "' class='companyListNameColumn'>" + company.name + "</td><td id='companyListTableSelector_" + companyId + "'><input type='text' id='companyListTableInput_" + companyId + "' value='" + (careerFairData.termVars.layout.tableMappings.get("companyId", companyId).tableNumber === null ? "" : careerFairData.termVars.layout.tableMappings.get("companyId", companyId).tableNumber) + "' size='2' maxlength='4'/></td></tr>");
    });
    //add null value
